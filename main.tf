  ...
  lb_rule_list = [
    {
      ...
      enable_tcp_reset               = false
      backend_address_pool_names       = [format("%s/%s","lb-ddi-dev","bkp-lb-ddi-dev" )] #[module.loadbalancer_backend_pool.lb_backend_address_pool_output["lb-ddi-dev/bkp-lb-ddi-dev"].id] #only Gateway SKU Load Balancer can have more than one "backend_address_pool_ids"
    },
    {
